...
export function fitCurve(points: IPoint3D[], maxError: number) {
  ...
  var add = (A: IPoint3D, B: IPoint3D) => {
    return { x: A.x + B.x, y: A.y + B.y, z: A.z + B.z }
  }
  var subtract = (A: IPoint3D, B: IPoint3D) => {
    return { x: A.x - B.x, y: A.y - B.y, z: A.z - B.z }
  }
  var multiply = (A: IPoint3D, B: number) => {
    return { x: A.x * B, y: A.y * B, z: A.z * B }
  }
  var divide = (A: IPoint3D, B: number) => {
    return { x: A.x / B, y: A.y / B, z: A.z / B }
  }
  ...
  var bezier = {
    ...
    qprime: (ctrlPoly: IPoint3D[], t: number) => {
      // 1st Derivative
      let tx = 1 - t
      return add(
        add(
    ...
    qprimeprime: (ctrlPoly: IPoint3D[], t: number) => {
      // 2nd Derivative
      return add(
        multiply(
          add(subtract(ctrlPoly[2], multiply(ctrlPoly[1], 2)), ctrlPoly[0]),
          6 * (1 - t),
        ),
        multiply(
          add(subtract(ctrlPoly[3], multiply(ctrlPoly[2], 2)), ctrlPoly[1]),
          6 * t,
        ),
      )
    },
  ...
  function fitCubic(
    points: IPoint3D[],
    leftTangent: IPoint3D,
    rightTangent: IPoint3D,
    error: number,
  ): IPoint3D[][] {
    var len = points.length
    ...
    for (let i = 0; i < len; ++i) u[i] /= u[len - 1]
    ...
    function computeMaxError(
      ...
      for (let i = 0; i < len; ++i) {
        var v = subtract(
          bezier.q(bez, find_t(parameters[i], tDistMap, bParts)),
          points[i],
        )
        var dist = dot(v, v)
        if (dist > maxDist) {
    ...
    function reparameterize(
      ...
      function newtonRaphsonRootFind(
        ...
        var denominator =
          dot(qprime, qprime) + 2 * dot(d, bezier.qprimeprime(bezCurve, u))

        return denominator === 0 ? u : u - numerator / denominator
      }

      return points.map((point: IPoint3D, i: number) =>
        newtonRaphsonRootFind(bezCurve, point, parameters[i]),
      )
    }
