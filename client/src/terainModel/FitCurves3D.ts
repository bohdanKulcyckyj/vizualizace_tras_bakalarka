...
*/
export function fitCurve(points: IPoint3D[], maxError: number) {

    // Remove duplicate points.
    points = points.filter((point, i) => (i === 0 || !(point.x === points[i - 1].x && point.y === points[i - 1].y && point.z === points[i - 1].z)));
    var len = points.length;
    if (len < 2) return [];

    // Simplified math.js functions used in this file.
    var add = (A: IPoint3D, B: IPoint3D) => { return { x: A.x + B.x, y: A.y + B.y, z: A.z + B.z }; };
    var subtract = (A: IPoint3D, B: IPoint3D) => { return { x: A.x - B.x, y: A.y - B.y, z: A.z - B.z }; };
    var multiply = (A: IPoint3D, B: number) => { return { x: A.x * B, y: A.y * B, z: A.z * B }; };
    var divide = (A: IPoint3D, B: number) => { return { x: A.x / B, y: A.y / B, z: A.z / B }; };
    var dot = (A: IPoint3D, B: IPoint3D) => A.x * B.x + A.y * B.y + A.z * B.z;
    var sum = (A: IPoint3D) => A.x + A.y + A.z;
    var norm = (A: IPoint3D) => Math.sqrt(dot(A, A));
    var normalize = (v: IPoint3D) => divide(v, norm(v));


    // Functions to evaluate a cubic bezier at t. Returns a point.
    var bezier = {
        q: (ctrlPoly: IPoint3D[], t: number) => { // 0th Derivative
            let tx = 1 - t;
            return add(
                add(
        ...
        qprime: (ctrlPoly: IPoint3D[], t: number) => { // 1st Derivative
            let tx = 1 - t;
            return add(
                add(
        ...
        qprimeprime: (ctrlPoly: IPoint3D[], t: number) => { // 2nd Derivative
            return add(
    ...
    function fitCubic(points: IPoint3D[], leftTangent: IPoint3D, rightTangent: IPoint3D, error: number): IPoint3D[][] {
        var len = points.length
        ...
        if (len === 2) {
            ...
            return [
                [points[0], add(points[0], multiply(leftTangent, dist)), add(points[1], multiply(rightTangent, dist)), points[1]]
            ];
        }
        ...
        function generateBezier(points: IPoint3D[], parameters: number[], leftTangent: IPoint3D, rightTangent: IPoint3D) {
            var len = points.length;
        ...
        function computeMaxError(points: IPoint3D[], bez: IPoint3D[], parameters: number[]) {
            var len = points.length;
            ...
            function find_t(param: number, tDistMap: number[], bParts: number): number {
                if (param <= 0) return 0;
                if (param >= 1) return 1;
        ...
        function reparameterize(bezCurve: IPoint3D[], points: IPoint3D[], parameters: number[]) {
            function newtonRaphsonRootFind(bezCurve: IPoint3D[], point: IPoint3D, u: number) {
            ...
            return points.map((point: IPoint3D, i: number) => newtonRaphsonRootFind(bezCurve, point, parameters[i]));
        }
        ...
        if ((centerVector.x === 0) && (centerVector.y === 0) && (centerVector.z === 0)) {
            centerVector = subtract(points[splitPoint - 1], points[splitPoint]);
            [centerVector.x, centerVector.y] = [-centerVector.y, centerVector.x];
        }
