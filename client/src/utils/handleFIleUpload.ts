...
export const handleFileUpload = ({ file, options }: IHandleFileUploadParams): Promise<AxiosResponse> => {
    return new Promise((resolve, reject) => {
        ...
        if (options.allowedFormats.length > 0 && !options.allowedFormats.includes(file.type)) {
            reject('File format is not allowed');
            return;
        }

        if (options.allowedSizeInMB > 0 && file.size / 1024 / 1024 >= options.allowedSizeInMB) {
        ...
        axios.post(apiEndpoints.uploadMedia, formData, requestConfig)
