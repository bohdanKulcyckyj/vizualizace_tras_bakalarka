...
const Table: FC<ITableProps> = ({ data, handleDelete, config }) => {
  ...
  const retrieveButton = (buttonData: IButton, rowData: any) => {
    switch (buttonData.type) {
      case ButtonType.REDIRECT:
        return <RedirectButton data={buttonData} rowData={rowData} />
      ...
      case ButtonType.UNPACK:
        return <UnpackButton data={buttonData} />
      default:
        return <button className='primary-button'>{buttonData.label}</button>
    }
  }
    ...
    return (
      <tr key={_rowIndex}>
        {config.tbody.map((_attribute, _dataIndex) => (
          <td key={_dataIndex}>{_data[_attribute]}</td>
        ))}
    ...
    if (showTheDialog) {
      toast(
        ...
        />,
        {
          ...
          unstyled: true,
          duration: 10000,
          onDismiss: () => setShowTheDialog(false),
        },
      )
    }
  }, [showTheDialog])
  ...
  return (
    <div className='table-component'>
      {(config.heading || config.newItemRoute) && (
        <div className='table-component__title flex justify-end my-4 mx-2'>
          {config.newItemRoute && (
