...
const PinPopup: React.FC<{
      ...
      <div className='confirm-dialog'>
        <div className='p-6 flex flex-col justify-center items-center'>
          <h2 className='confirm-dialog__title'>
            Do you really want to remove this point?
          </h2>
  ...
  useEffect(() => {
    setTimeout(() => {
      const inputWithAutoFocus = document.querySelector<HTMLInputElement>(
        'input[name="point-label"]',
      )
                ...
                Object.values(PIN_COLORS).map((_value, _index) => {
                  return (
                    <div
                      key={_index}
                      className={
                        (String(formState.color) == _value.toString()
                          ? 'active '
                          : '') + 'cursor-pointer'
                      }
                      onClick={() =>
